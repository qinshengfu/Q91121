 ...
 */
public class AddDynamicTask extends TimerTask {

    @Override
    public void run() {
        try {
            //在这里写你要执行的内容
            System.out.println("--------------->>添加动态奖励定时任务");
            ...
            for (PageData map : list) {
                ...
                name = Const.STATIC_REWARD_TASK + id;
                cron = DateUtil.getCron(releaseTime);
                // 设置参数
                Map<String, Object> parameter = new HashMap<String, Object>();
                parameter.put("orderId", id);
                QuartzManager.addJob(name, DynamicBonusRelease.class, cron, parameter);
                i++;
            }
            System.out.println("动态奖励释放定时任务添加完毕共"+ i +"个任务");
            context.close();
        } catch (Exception e) {
