			...
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						...
						<!-- 检索  -->
						<form action="sell_smd/list.do" method="post" name="Form" id="Form">
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
								...
								<tr>
									...
									<th class="center">提现数额</th>
									<th class="center">余额</th>
									<th class="center">手机号</th>
									<th class="center">接收类型</th>
									...
									<c:when test="${not empty varList}">
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													<td class='center'>
														<label class="pos-rel"><input type='checkbox' name='ids' value="${var.SELL_SMD_ID}" class="ace" /><span class="lbl"></span></label>
													</td>
													...
													<td class="center">
														...
														<div class="hidden-sm hidden-xs btn-group">
															<c:if test="${var.NUMBER == var.BALANCE}">
																<c:if test="${QX.del == 1 }">
																	<a class="btn btn-xs btn-danger" onclick="del('${var.SELL_SMD_ID}');">
															...
															<div class="inline pos-rel">
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	<c:if test="${var.NUMBER == var.BALANCE}">
																		<c:if test="${QX.del == 1 }">
																			<li>
																				<a style="cursor:pointer;" onclick="del('${var.SELL_SMD_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																		<span class="red">
...
<script type="text/javascript">
	...
	function add(){
		...
		diag.Drag=true;
		diag.Title ="新增";
		diag.URL = '<%=basePath%>sell_smd/goAdd.do';
		diag.Width = 450;
		diag.Height = 355;
	...
	function del(Id){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>sell_smd/delete.do?SELL_SMD_ID="+Id+"&tm="+new Date().getTime();
				$.get(url,function(data){
					if (data === "success"){
						bootbox.confirm("这笔提现订单已删除", function(result) {
							if (result) {
								tosearch();
							}
						});
					} else {
						bootbox.confirm("删除失败:【"+ data +"】", function(result) {
							if (result) {
								tosearch();
							}
						});
	...
	function edit(Id){
		...
		diag.Drag=true;
		diag.Title ="编辑";
		diag.URL = '<%=basePath%>sell_smd/goEdit.do?SELL_SMD_ID='+Id;
		diag.Width = 450;
		diag.Height = 355;
		diag.Modal = true;				//有无遮罩窗口
		diag. ShowMaxButton = true;	//最大化按钮
			...
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						...
						$.ajax({
							type: "POST",
							url: '<%=basePath%>sell_smd/deleteAll.do?tm='+new Date().getTime(),
							data: {DATA_IDS:str},
							dataType:'json',
	...
	//导出excel
	function toExcel(){
		window.location.href='<%=basePath%>sell_smd/excel.do';
	}
</script>
