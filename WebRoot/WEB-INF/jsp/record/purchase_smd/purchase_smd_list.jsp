			...
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						...
						<!-- 检索  -->
						<form action="purchase_smd/list.do" method="post" name="Form" id="Form">
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
								...
								<tr>
									...
									<th class="center">打款方式</th>
									<th class="center">利息周期(天数)</th>
											...
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													...
													<c:if test="${var.ORDER_TYPE == 1}">
														<td class='center'>预付款</td>
													</c:if>
													<c:if test="${var.ORDER_TYPE == 2}">
														<td class='center'>尾款</td>
													</c:if>
													...
													<td class='center'>${var.INTEREST_CYCLE}</td>
													<td class='center'>${var.PAYMENT_TIME}</td>
													<c:if test="${var.PAYMENT_STATE == 5}">
														<td class='center'>等待匹配</td>
													</c:if>
													...
													<td class="center">
														...
														<div class="hidden-sm hidden-xs btn-group">
															<c:if test="${var.PAYMENT_STATE == 5}">
																<c:if test="${QX.del == 1 }">
																	<a class="btn btn-xs btn-danger" onclick="del('${var.ORDER_NUMBER}', '${var.ORDER_TYPE}');">
														...
														<div class="hidden-md hidden-lg">
															<div class="inline pos-rel">
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	<c:if test="${QX.del == 1 }">
																		<li>
																			<a style="cursor:pointer;" onclick="del('${var.ORDER_NUMBER}', '${var.ORDER_TYPE}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
<script type="text/javascript">
	...
	function del(Id, type){
		...
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>purchase_smd/delete.do?ORDER_NUMBER="+Id+"&ORDER_TYPE=" + type;
				$.get(url,function(data){
					if (data === "success"){
						bootbox.confirm("这笔订单的预付款和尾款已删除", function(result) {
							if (result) {
								tosearch();
							}
						});
	...
	function edit(Id){
		...
		diag.Drag=true;
		diag.Title ="编辑";
		diag.URL = '<%=basePath%>purchase_smd/goEdit.do?PURCHASE_SMD_ID='+Id;
		diag.Width = 450;
		diag.Height = 355;
			...
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						...
						$.ajax({
							type: "POST",
							url: '<%=basePath%>purchase_smd/deleteAll.do?tm='+new Date().getTime(),
							data: {DATA_IDS:str},
							dataType:'json',
	...
	//导出excel
	function toExcel(){
		window.location.href='<%=basePath%>purchase_smd/excel.do';
	}
</script>
